...
first_function_input:
  KW_IDENTIFIER KW_DOUBLE_DOT space data_type  //x: type
  { $$=template("%s %s", $4, $1); }            //typeC x
  | kw_arrays KW_DOUBLE_DOT space data_type    //array[num]: type
  { $$=template("%s %s",$4, $1); }             //typeC array[num]
//function input variables
function_inputs:       
  first_function_input
  | first_function_input KW_COMMA space function_inputs//x: type,%space ...
  { $$ = template("%s, %s",$1, $4);} 		       //typeC x,...
//function begining-declaration
functions_begin:  
  KW_FUNCTION_START space KW_IDENTIFIER KW_LEFT_BRACKET function_inputs KW_RIGHT_BRACKET KW_FUNCTION_RETURN_TYPE data_type KW_DOUBLE_DOT //def foo(inputs)->return type:
    { $$ = template("%s %s(%s){", $8, $3, $5); }											 //return type foo(inputs){
  | KW_FUNCTION_START space KW_IDENTIFIER KW_LEFT_BRACKET function_inputs KW_RIGHT_BRACKET KW_DOUBLE_DOT                                 //def foo(inputs):
    { $$ = template("void %s(%s){", $3, $5);}												 //void foo(inputs){
  | KW_FUNCTION_START space KW_IDENTIFIER KW_LEFT_BRACKET  KW_RIGHT_BRACKET KW_DOUBLE_DOT						 //def foo():
    { $$ = template("void %s(){", $3);}													 //void foo(){
  | KW_FUNCTION_START space KW_IDENTIFIER KW_LEFT_BRACKET  KW_RIGHT_BRACKET KW_FUNCTION_RETURN_TYPE data_type KW_DOUBLE_DOT              //def foo()->return:
...
//identifiers
kw_identifiers:
  kw_identifiers KW_COMMA space  kw_identifiers { $$ = template("%s, %s", $1, $4); } //x,.. y
  | KW_IDENTIFIER  {$$ = template("%s", $1); }                                       //x
//arrays
kw_arrays:
  KW_IDENTIFIER KW_LEFT_ARRAY KW_INTEGER KW_RIGHT_ARRAY {$$ = template("%s[%s]", $1,$3);}                                     //array[num]
  | kw_arrays KW_COMMA space  KW_IDENTIFIER KW_LEFT_ARRAY KW_INTEGER KW_RIGHT_ARRAY {$$ = template("%s, %s[%s]", $1, $4,$6);} //array[num], ..., arraySecond[num2] 
//variables declarations
input_variables:
   kw_identifiers KW_DOUBLE_DOT space data_type KW_SEMICOLON { $$ = template("%s %s;\n", $4, $1);} //x, y, z: type
  | kw_arrays KW_DOUBLE_DOT space data_type KW_SEMICOLON { $$ = template("%s %s;\n", $4, $1);}     //array[num], array[num2]: type

/*==================== Constants ====================*/
constants:
   KW_CONSTANT space KW_IDENTIFIER space KW_EQUAL space KW_INTEGER space KW_DOUBLE_DOT data_type KW_SEMICOLON //const identifier_type = value :data_type;
  { $$ = template("const %s %s = %s;\n", $10, $3, $7); }						      //const data_type identifier_type = value;
  | KW_CONSTANT space KW_IDENTIFIER space KW_EQUAL space KW_STRING space KW_DOUBLE_DOT data_type KW_SEMICOLON
  { $$ = template("const %s %s = %s;\n", $10, $3, $7); }
  | KW_CONSTANT space KW_IDENTIFIER space KW_EQUAL space KW_BOOLEAN space KW_DOUBLE_DOT data_type KW_SEMICOLON
  { $$ = template("const %s %s = %s;\n", $10, $3, $7); }
  | KW_CONSTANT space KW_IDENTIFIER space KW_EQUAL space KW_REAL space KW_DOUBLE_DOT data_type KW_SEMICOLON
  { $$ = template("const %s %s = %s;\n", $10, $3, $7); }
...
if_begining:
  KW_IF space KW_LEFT_BRACKET expression KW_RIGHT_BRACKET  KW_DOUBLE_DOT //if (expression):
  {$$ = template("if(%s){\n", $4);}					 //if(expression){
//else case
else:
...
for_statement_beginning:
  KW_FOR space KW_IDENTIFIER space KW_IN space KW_LEFT_ARRAY KW_INTEGER 
...
while_statement_beginning:
  KW_WHILE space KW_LEFT_BRACKET expression KW_RIGHT_BRACKET KW_DOUBLE_DOT //while (expression):
  { $$ = template("while(%s){\n", $4);}					   //while(expression){
...
return:
  ...
  | KW_RETURN space expression KW_SEMICOLON {$$=template("return %s;\n", $3);}	//return expression;
...
/*==================== Main() ====================*/
main: 
  KW_FUNCTION_START space KW_MAIN KW_LEFT_BRACKET KW_RIGHT_BRACKET KW_DOUBLE_DOT //def main():
  {$$ = template("int main(){");}						 //itn main(){

/*==================== Space ====================*/
space:
  space KW_SPACE {$$=template("%s%s", $1,$2);}
  | KW_SPACE {$$ = template("%s", $1);}
